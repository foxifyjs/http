...
describe(".download(path)", () => {
  it("should transfer as an attachment", async () => {
    ...
    const result = await inject(
      (req, res) => res.download("__tests__/__internals__/fixtures/user.html"),
      "/",
    );
...
describe(".download(path, filename)", () => {
  it("should provide an alternate filename", async () => {
    ...
    const result = await inject(
      (req, res) =>
        res.download("__tests__/__internals__/fixtures/user.html", "document"),
      "/",
    );
...
describe(".download(path, fn)", () => {
  it("should invoke the callback", async () => {
    ...
    const result = await inject(
      (req, res) =>
        res.download("__tests__/__internals__/fixtures/user.html", cb),
      "/",
    );
...
describe(".download(path, filename, fn)", () => {
  it("should invoke the callback", async () => {
    ...
    const result = await inject(
      (req, res) =>
        res.download(
          "__tests__/__internals__/fixtures/user.html",
          "document",
          cb,
        ),
      "/",
...
describe(".download(path, filename, options, fn)", () => {
  ...
  it("should allow options to res.sendFile()", async () => {
    ...
    const result = await inject(
      (req, res) =>
        res.download("__tests__/__internals__/fixtures/.name", "document", {
          dotfiles: "allow",
          maxAge: "4h",
        }),
      "/",
...
describe("on failure", () => {
  it("should invoke the callback", async () => {
    ...
    const result = await inject(
      (req, res) =>
        res.download("__tests__/__internals__/fixtures/foobar.html", err => {
